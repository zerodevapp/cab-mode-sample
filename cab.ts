...
const main = async () => {
  ...
  await cabClient.enableCAB({
    tokens: [{ name: "USDC", networks: [mode.id] }],
  });
  ...
  const userOpHash = await cabClient.sendUserOperationCAB({ userOperation });
  console.log("userOpHash", userOpHash);
  const ret = await bundlerClient.waitForUserOperationReceipt({
    hash: userOpHash,
    timeout: 100_000_000,
  });
  console.log(
    "transfer with cab transactionHash:",
